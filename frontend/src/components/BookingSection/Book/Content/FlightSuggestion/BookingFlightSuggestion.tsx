...
const BookingFlightSuggestion: React.FC<Props> = ({flightArea, handleSetupDisplaySuggestion}) => {
  ...
  return (
    <div className = "z-50 absolute top-[43.5rem] bg-white h-[30.45rem] overflow-y-scroll setting-scrollbar overflow-x-hidden" style = {{left: "33rem", width: "23rem"}}>
        ...
        <div className = "absolute top-[0rem] right-[0rem]">
            <IoMdClose className = "w-[1.5rem] h-[1.5rem] hover:cursor-pointer" style = {{color: "gray"}} onClick = {() => handleSetupDisplaySuggestion(false)}/>
        </div>
    </div>
  )
}
