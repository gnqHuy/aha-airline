...
const CardComponent: React.FC<CardProps> = ({ image, title, link, width = "500px" }) => {
  return (
    ...
    >
      ...
      <div className="absolute text-xl bottom-0 left-0 right-0 bg-ahaGreen-1 bg-opacity-70 text-white font-bold px-4 pt-4 pb-5 flex justify-between items-center">
        <span>{title}</span>
        <IoIosArrowForward />
      </div>
    </Link>
